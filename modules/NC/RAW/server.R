nc_raw_server <- function(id, activated){
  ...
  moduleServer(id, function(input, output, session) {
    ...
    initialize_module <- function() {
      if (initialized) {
        WARN("Module 'nc-raw' already initialized")
        return(invisible(NULL))
      }
      INFO("Initialize module 'nc-raw'")
      initialized <<- TRUE
      
      # These should ideally be sourced once at application startup.
      # If they must be sourced here, do it only once per activation.
      ...
      react_ref(
        initialize_NC_reference_data(
          iotc.data.reference.datasets.NC::RAW
        )
      )
      
      # IMPORTANT:
      # prepare_NC_handlers() must return the observers it creates.
      prepare_NC_handlers(
    ...
    session$onSessionEnded(function() {
      message("Session ended for NC-RAW module")
